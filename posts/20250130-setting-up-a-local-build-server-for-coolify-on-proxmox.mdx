...
In the last article we set up Coolify on an LXC in Proxmox and secured everything with Tailscale. Now we'll set up another LXC in Proxmox to act as our build server. Because why _wouldn't_ we?
